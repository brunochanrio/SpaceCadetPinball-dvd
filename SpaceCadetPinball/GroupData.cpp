...
void GroupData::SplitSplicedBitmap(const gdrv_bitmap8& srcBmp, gdrv_bitmap8& bmp, zmap_header_type& zMap)
{
	...
	for (int dstInd = 0;;)
	{
		auto stride = static_cast<int16_t>(utils::swap_u16(*src++));
		if (stride < 0)
			break;
		...
		dstInd += stride;
		for (auto count = utils::swap_u16(*src++); count; count--)
		{
			auto depth = utils::swap_u16(*src++);
...
MsgFont *DatFile::ReadPEMsgFontResource(const std::string peName)
{
	...
	sizeOfOptionalHeader = utils::swap_u16(sizeOfOptionalHeader);
	...
	rsrcSectionVirtualAddress = utils::swap_u32(rsrcSectionVirtualAddress);
	...
	rsrcSectionDataOffset = utils::swap_u32(rsrcSectionDataOffset);

	fseek(fileHandle, rsrcSectionDataOffset + 0xe, SEEK_SET);
	uint16_t numberOfIDEntries;
	fread(&numberOfIDEntries, sizeof(uint16_t), 1, fileHandle);
	numberOfIDEntries = utils::swap_u16(numberOfIDEntries);
	...
	for (uint16_t e = 0; e < numberOfIDEntries; e++)
	{
		...
		entryIntegerID = utils::swap_i32(entryIntegerID);

		if (entryIntegerID == 0xa) // RCDATA
		{
			fread(&subdirectoryOffset, sizeof(uint32_t), 1, fileHandle);
			subdirectoryOffset = utils::swap_u32(subdirectoryOffset);
	...
	numberOfNameEntries = utils::swap_u16(numberOfNameEntries);
	...
	nameOffset = utils::swap_u32(nameOffset);
	...
	for (int n = 0; n < 9; n++)
	{
		if (utils::swap_u16(name[n]) != expectedName[n])
		{
			fclose(fileHandle);
			return nullptr;
		}
	...
	languageID = utils::swap_u32(languageID);*/
	...
	dataEntryOffset = utils::swap_u32(dataEntryOffset);
	...
	dataOffset = utils::swap_u32(dataOffset);
	fread(&dataSize, sizeof(uint32_t), 1, fileHandle);
	dataSize = utils::swap_u32(dataSize);
